...
export function cleanupDeadClientJob(connection: ConnectionManager) {
  setInterval(async () => {
    await findDeadClients(connection);
  }, 60000 * 5);
}

async function findDeadClients(connection: ConnectionManager) {
